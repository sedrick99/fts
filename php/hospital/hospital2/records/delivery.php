...
<body>
    ...
    <header>ST. THERESE MEDICAL CENGTER NOMAYOS</header>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add'])) {
    ...
    $sql = "SELECT * FROM delivery WHERE name ='$name'";
$result = $conn->query($sql);
    if ($result->num_rows === 0) {

    $sql = "INSERT INTO delivery (name, amount, date, hospital_id) VALUES ('$name','$amount','$date', '$hospital_id')";
    ...
    $sql = "UPDATE delivery SET amount = '$amount' WHERE name = '$name'";
    if ($conn->query($sql) === TRUE) {
