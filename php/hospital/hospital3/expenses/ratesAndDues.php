...
<body>
    ...
    <header>St. ZELIE AND LOUIS MEDICAL CENTER</header>
  ...
  <section>
    ...
    <div class="main-products">
        <form class="man" method="post">
            <h1>Add Rates And Dues</h1>
            <label for="name">Rates Name</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add'])) {
    ...
    $sql = "SELECT * FROM rates WHERE name ='$name'";
$result = $conn->query($sql);
    if ($result->num_rows === 0) {

    $sql = "INSERT INTO rates (name, amount, date, hospital_id) VALUES ('$name','$amount','$date', '$hospital_id')";
    ...
    $stmt = $conn->prepare("UPDATE rates SET amount = amount + ? WHERE name = ?");
