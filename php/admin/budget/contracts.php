...
<body>
    ...
    <div class="main-products">
        <form class="man" method="post">
            ...
            <input type="text" id="name" name="name" placeholder="Enter Contract Name" required>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add'])) {
    ...
    $sql = "SELECT * FROM contracts WHERE name ='$name'";
$result = $conn->query($sql);
    if ($result->num_rows === 0) {

    $sql = "INSERT INTO contracts (name, amount, date) VALUES ('$name','$amount','$date')";

    if ($conn->query($sql) === TRUE) {
        echo '<div class="success-message" id="success-message">Contract added successfully!</div>';

        exit();
    }
}else{
    echo '<div class="success-message" id="success-message"> Contract Already Exist!</div>';
